 ...
 *
 * \brief  The RobotKE class represents an Knee Exo Robot.
 *
 */

#ifndef RobotKE_H_INCLUDED
#define RobotKE_H_INCLUDED
...
typedef struct KETool
{
    KETool(double l, double m, std::string n="tool"):length(l),mass(m),name(n) {};
...
static KETool KENoTool(0, .0, "No Tool"); //!< No Tool
static KETool KEHandle(0.140, 0.800, "Handle"); //!< Default handle with 3 rotational DoFs 0.465 //TODO YAML

/**
 * \brief Implementation of the M3 robot class, representing an M3 using 3 JointKE (and so Kinco drives).
 * model reference:
 *
 *                              /\
 *                            /-  \
...
class RobotKE : public Robot {
   ...
   public:
    /**
      * \brief Default RobotKE constructor.
      ...
      */
    RobotKE(std::string robot_name="", std::string yaml_config_file="");
    ~RobotKE();
    ...
    */
    ...
    setMovementReturnCode_t setJointPosition(double q);
    setMovementReturnCode_t setJointVelocity(double dq);
    setMovementReturnCode_t setJointTorque(double tau);
    setMovementReturnCode_t setEndEffForceWithCompensation(VM3 F, bool friction_comp=true);

    void changeTool(KETool *new_tool) {endEffTool=new_tool; std::cout << "RobotKE::changeTool: new tool: " << endEffTool->name << std::endl;}
};
#endif /*RobotKE_H*/
